...
public class FilesplittAction implements Action {
	...
	public void doAction(Properties parameters, Storage storage, BackupJob job,
		...
		try {
			PriorityQueue<DataObject> sorted = new PriorityQueue<>(
					storage.getDataObjectCount(), new Comparator<DataObject>() {
						@Override
			...
			while (dataobjects.hasNext() == true) {
				...
				System.out.println("New File Path: " + PATH_SEPARATOR + tmp_dir
						+ daob.getPath());

				storage.move(daob.getPath(),
						PATH_SEPARATOR + tmp_dir + daob.getPath());
			}
			dataobjects = null;
			...
			if (parameters.containsKey(PROP_MAX_CONTAINER_SIZE)) {
				if (parameters.getProperty(PROP_MAX_CONTAINER_SIZE) == "-1") {
					...
					System.out
							.println("Property is -1. Set container to maximum: "
									+ CONTAINER_MAX_SIZE);
				} else {
					Long maxsize = new Long(
							parameters.getProperty(PROP_MAX_CONTAINER_SIZE));
					fcs = new FileContainers(maxsize, true);
					System.out.println("Property is set. Set containersize: "
							+ parameters.getProperty(PROP_MAX_CONTAINER_SIZE));
				}
			} else {
				fcs = new FileContainers(CONTAINER_MAX_SIZE, true);
				System.out
						.println("Property is not available. Set container to maximum: "
								+ CONTAINER_MAX_SIZE);
				// throw new ActionException ("Property \"" +
				// PROP_MAX_CONTAINER_SIZE + "\" is not set");
			}
			...
			parameters.setProperty("org.backmeup.filesplitting.containercount",
					fcs.getContainers().size() + "");

			int container = 0;
			for (FileContainer fc : fcs.getContainers()) {
				parameters.setProperty("org.backmeup.filesplitting.container."
						+ container + ".size", fc.getContainersize() + "");
				parameters.setProperty("org.backmeup.filesplitting.container."
						+ container + ".name", fc.getContainerpath());
				container++;
				...
				for (int i = 0; i < fc.getContainerElementCount(); i++) {
					System.out.println("Old File Path: "
							+ fc.getContainerElementOldPath(i));
					System.out.println("New File Path: "
							+ fc.getContainerElementNewPath(i).replaceAll(
									tmp_dir + PATH_SEPARATOR, ""));

					// TODO remove the tmp folder with new storage interface
					// (existFolder)
					storage.move(
							fc.getContainerElementOldPath(i),
							fc.getContainerElementNewPath(i).replaceAll(
