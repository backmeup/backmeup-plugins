...
public class FilesplittAction implements Action {
    ...
    public void doAction(Properties accessData, Properties parameters, List<String> options, Storage storage,
        ...
        try {
            PriorityQueue<DataObject> sorted = new PriorityQueue<>(storage.getDataObjectCount(),
            ...
            if (parameters.containsKey(this.PROP_MAX_CONTAINER_SIZE)) {
                if (parameters.getProperty(this.PROP_MAX_CONTAINER_SIZE) == "-1") {
                    ...
                    Long maxsize = new Long(parameters.getProperty(this.PROP_MAX_CONTAINER_SIZE));
                    fcs = new FileContainers(maxsize, true);
                    System.out.println("Property is set. Set containersize: "
                            + parameters.getProperty(this.PROP_MAX_CONTAINER_SIZE));
                }
            } else {
                fcs = new FileContainers(CONTAINER_MAX_SIZE, true);
            ...
            for (FileContainer fc : fcs.getContainers()) {
                parameters.setProperty("org.backmeup.filesplitting.container." + container + ".size",
                        fc.getContainersize() + "");
                parameters.setProperty("org.backmeup.filesplitting.container." + container + ".name",
                        fc.getContainerpath());
                container++;
                ...
                for (int i = 0; i < fc.getContainerElementCount(); i++) {
                    ...
                    System.out.println("New File Path: "
                            + fc.getContainerElementNewPath(i).replaceAll(tmp_dir + PATH_SEPARATOR, ""));

                    // TODO remove the tmp folder with new storage interface
                    // (existFolder)
                    storage.move(fc.getContainerElementOldPath(i),
                            fc.getContainerElementNewPath(i).replaceAll(tmp_dir + PATH_SEPARATOR, ""));
                }
            }
        } catch (Exception e) {
