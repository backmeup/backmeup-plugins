...
public class FileContainers {
	...
	private String getNewContainerpath() throws ActionException {
		...
		if (encrypt == false) {
			containerpath = "part"
					+ String.format(PART_NUM_FORMAT, filecontainers.size() + 1);
		} else {
			containerpath = "part"
					+ String.format(PART_NUM_FORMAT, filecontainers.size() + 1)
					+ CONTAINER_CRYPT_EXTENSION;
		}

		if (filecontainers.size() > CONTAINER_LIMIT) {
			// TODO throw new ToMuchSplitt...
			throw new ActionException("Can't split data in more than "
					+ CONTAINER_LIMIT + " containers");
		}
	...
	public void addData(DataObject data) throws ActionException {
		if (data.getLength() > containermaxsize) {
			...
			throw new ActionException("File (path: " + data.getPath()
					+ ", size: " + data.getLength()
					+ ") is bigger than maximum container size ("
					+ containermaxsize + ").");
		}

		if (currcontainer == null) {
			currcontainer = new FileContainer(getNewContainerpath(),
					containermaxsize);
		}
