...
public class SftpAuthenticator implements InputBasedAuthorizable {
    ...
    public List<RequiredInputField> getRequiredInputFields() {
        ...
        inputs.add(
                new RequiredInputField(PROP_HOST, PROP_HOST, PROP_HOST_DESC, true, 0, Type.String, PROP_HOST_DEFAULT));
        inputs.add(
                new RequiredInputField(PROP_PORT, PROP_PORT, PROP_PORT_DESC, true, 1, Type.Number, PROP_PORT_DEFAULT));
        inputs.add(new RequiredInputField(PROP_FOLDER, PROP_FOLDER, PROP_FOLDER_DESC, true, 2, Type.String,
                PROP_FOLDER_DEFAULT));
        inputs.add(new RequiredInputField(PROP_USERNAME, PROP_USERNAME, PROP_USERNAME_DESC, true, 3, Type.String,
                PROP_USERNAME_DEFAULT));
        inputs.add(new RequiredInputField(PROP_PASSWORD, PROP_PASSWORD, PROP_PASSWORD_DESC, true, 4, Type.Password,
                PROP_PASSWORD_DEFAULT));
        return inputs;
    }
    ...
    private void addEntryIfKeyMissing(Map<String, String> properties, String key, ValidationNotes notes) {
        if (!properties.containsKey(key)) {
            notes.addValidationEntry(ValidationExceptionType.ConfigException, SftpDescriptor.SFTP_ID,
