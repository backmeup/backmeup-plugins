...
/**
 * The indexer must be handed an ElasticSearch client in order to work. (This
 * must be done by the class orchestrating the backup workflow!) Cf.
 * IndexActionTest for an example on how to create a client talking to an ad-hoc
 * embedded ElasticSearch node.
 * 
 * To talk to an existing ElasticSearch cluster, I recommend using a
 * TransportClient, like so:
 * 
 ...
 * 
 * It is possible to add arbitrary numbers of transport addresses - the client
 * will communicate with them in round-robin fashion.
 * 
...
public class ElasticSearchIndexer {
	...
	public void doIndexing(BackupJob job, DataObject dataObject,
		...
		document.field(IndexFields.FIELD_OWNER_NAME, job.getUser()
				.getUsername());
		document.field(IndexFields.FIELD_FILENAME,
				getFilename(dataObject.getPath()));
		document.field(IndexFields.FIELD_PATH, dataObject.getPath());
		document.field(IndexFields.FIELD_FILE_HASH, dataObject.getMD5Hash());
		document.field(IndexFields.FIELD_BACKUP_SINK, job.getSinkProfile()
				.getName());
		...
		if (sourceProfile != null) {
			document.field(IndexFields.FIELD_BACKUP_SOURCE_ID,
					sourceProfile.getId());
			document.field(IndexFields.FIELD_BACKUP_SOURCE_PLUGIN_NAME,
					sourceProfile.getPluginId());
			document.field(IndexFields.FIELD_BACKUP_SOURCE_IDENTIFICATION,
					sourceProfile.getIdentification());
		}
		...
		if (metainfoContainer != null) {
			...
			while (it.hasNext()) {
				...
				for (Object key : metainfo.keySet()) {
					document.largeField(key.toString(), metainfo.get(key)
							.toString());
				}
			}
		}
