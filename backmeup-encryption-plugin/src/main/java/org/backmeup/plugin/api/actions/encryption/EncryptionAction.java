...
public class EncryptionAction implements Action {
	...
	public void doAction(Properties parameters, Storage storage, BackupJob job,
		...
		if (parameters.containsKey(PROP_CONT_COUNT) == true) {
			...
			throw new ActionException("Property \"" + PROP_CONT_COUNT
					+ "\" is not set");
		}
		...
		for (int i = 0; i < containers; i++) {
			if (parameters.containsKey("org.backmeup.filesplitting.container."
					+ i + ".size") == true) {
				containersize[i] = new Long(
						parameters
								.getProperty("org.backmeup.filesplitting.container."
										+ i + ".size"));
				...
				throw new ActionException(
						"Property \"org.backmeup.filesplitting.container." + i
								+ ".size\" is not set");
			}

			if (parameters.containsKey("org.backmeup.filesplitting.container."
					+ i + ".name") == true) {
				containername[i] = parameters
						.getProperty("org.backmeup.filesplitting.container."
								+ i + ".name");
			} else {
				throw new ActionException(
		...
		if (parameters.containsKey(PROP_PASSWORD) == true) {
			...
			throw new ActionException("Property \"" + PROP_PASSWORD
					+ "\" is not set");
		}

		EncryptionContainers enccontainers = new EncryptionContainers(
				containers, containersize, containername, password);
		try {
		...
		for (EncryptionContainer container : enccontainers.getContainers()) {
			try {
				...
				storage.addFile(container.getContainer(),
						container.getContainername(), new MetainfoContainer());
