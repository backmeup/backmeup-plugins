...
public class EncryptionTcManager {
	public void createContainer(EncryptionContainer container) throws ActionException {
		ProcessBuilder pb = new ProcessBuilder("sudo",
				"/usr/local/sbin/create_container.sh",
				container.getContainerpath(), container.getMountpoint(),
				container.getSize() + "", container.getPassword());
		try {
	...
	public void unmountContainer(EncryptionContainer container) throws ActionException {
		ProcessBuilder pb = new ProcessBuilder("sudo",
				"/usr/local/sbin/umount_container.sh",
