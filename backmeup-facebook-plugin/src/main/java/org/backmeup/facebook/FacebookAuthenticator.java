...
public class FacebookAuthenticator implements OAuthBasedAuthorizable {
    ...
    public String createRedirectURL(Map<String, String> inputProperties, String callback) {
         ...
         * FacebookHelper.getAppKey() + "&redirect_uri=" + callback + "&scope="
         * +
    ...
    private String retrieveAccessToken(Map<String, String> inputProperties) {
         ...
         * FacebookHelper.getAppKey() + "&redirect_uri=" + callback +
         * "&client_secret=" + FacebookHelper.getAppSecret() + "&code=" + code);
         * 
         * try (BufferedReader reader = new BufferedReader(new
         * InputStreamReader(url.openStream(), Charset.forName("UTF-8")))) { int
         * temp;
         * 
         * while ((temp = reader.read()) != -1) { content.append((char) temp); }
         * } } catch (IOException ex) { throw new
         * PluginException(FacebookDescriptor.ID, AUTHENTICATION_ERROR, ex); }
         * 
         * if (content.toString().contains("access_token")) { String[] params =
         * content.toString().split("&"); for (String param : params) { String[]
         * kvPair = param.split("="); String name = kvPair[0]; if
         * (name.equals("access_token")) { return kvPair[1]; } } }
         * 
         * throw new PluginException(FacebookDescriptor.ID,
